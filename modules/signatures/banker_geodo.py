...
class Geodo(Signature):
    ...
    def run(self):
        ...
        match_runkey = self.check_key(pattern="HKEY_CURRENT_USER\\\\Software\\\\Microsoft\\\\Windows\\\\CurrentVersion\\\\Run\\\\[a-z]{3}(api32|audio|bios|boot|cap32|common|config|crypt|edit32|error|mgr32|serial|setup|share|sock|system|update|video|windows)\.exe$", regex=True, all=True)
        match_otherkey = self.check_key(pattern="HKEY_CURRENT_USER\\\\Software\\\\Microsoft\\\\Office\\\\Common\\\\(?P<hex>[A-F0-9]+)\\\\(?P=hex)(CS|PS|SS|RS)", regex=True, all=True)
        match_mutex = self.check_mutex(pattern="^[A-F0-9]{1,8}(I|M)$", regex=True, all=True)
